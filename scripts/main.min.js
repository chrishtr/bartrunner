...
const BartRunner = function(root) {
  ...
  function addStationsTo(selector) {
    ...
    request.open('GET', 'https://api.bart.gov/api/stn.aspx?cmd=stns&key=MW9S-E7SL-26DU-VV8V&json=y', true);
...
//            case .msWithSpace:
            return signStr + wholeMinutes + "m " + pad(wholeSeconds,2) + "s"
//            case .mOnly:
...
//            }
  ...
  function getEtaDispStr(etaMinutes) {
    ...
    } else if (etaMinutes < 0)
    {
        ...
        return formatTimeMinutes(etaMinutes);
    }
  }

  function displayTrainStatus(trainStatus) {
   ...
   // sort by departure time
    ...
    trainStatus.forEach(etd => {
      ...
      leavingBoxSpan.innerHTML = getEtaDispStr(thisEtdVal);
      ...
      arrivingBoxSpan.innerHTML = getEtaDispStr(thisEtaVal);
  ...
  function updateTrains() {
    var request = new XMLHttpRequest();
    request.open('GET', 'https://api.bart.gov/api/etd.aspx?cmd=etd&orig=' + fromStation.value + '&key=MW9S-E7SL-26DU-VV8V&json=y', true);
    request.onload = function () {
      var data = JSON.parse(this.response);
      if (request.status >= 200 && request.status < 400) {
