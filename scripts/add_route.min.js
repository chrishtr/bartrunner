...
class BartRunnerAddRoute {
  constructor(root) {
    ...
    let okButton = root.querySelector("#ok-button");
    let cancelButton = root.querySelector("#cancel-button");
    let that = this;
    okButton.onclick = function() {
      let fromVal = that.fromStation.value;
      let toVal = that.toStation.value;
      let newFav = {"from": fromVal, "to": toVal};
      let currentFavorites = BartRunnerUtils.loadFavorites();
      if ((!currentFavorites) || !(currentFavorites instanceof Array)) {
        currentFavorites = [];
      }
      currentFavorites.push(newFav);
      BartRunnerUtils.saveFavorites(currentFavorites);
